...
### Added
- **The rest of the interface, server messages, and notifications are now translatable (Spanish included).** Every screen, activity/history entries, job errors, API errors and detection notes follow your UI language, and a new Settings → System → Notifications language setting controls outbound Discord/webhook/email text; history recorded before this version stays English.
...
### Fixed
- **Scanner: "Select all" then deselecting one item no longer clears the whole selection.** Deselecting under select-all rebuilt the selection from only the expanded/loaded folders, dropping every unloaded item; it now starts from the full known set (all folder prefixes, or the advanced-search matches) and removes just the one item.
...
### Fixed
- **Bulk language detection now processes titles in alphabetical order.** It followed DB rowid (scan) order, which drifts from alphabetical after multi-pass scans and made ignored/foreign titles look like they were left for last; the batch is now sorted by path.
...
### Added
- **Language detection reads the track title.** A language named in a track's title ("Traditional Chinese", "Romanian", "English SDH") is now used directly, tried before content/OCR detection. Fixes `und` forced/SDH subtitles that carry too little text for langdetect but name the language in their title. Applies to audio and subtitle tracks.
